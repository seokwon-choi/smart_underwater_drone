...
void Processor_pop0_400(void){ // return F_ACT_400
	if(check_pop0()){	
		setflag(F_COMTIME_400);
		timercom_400=0; // 다시 시간 카운트
		switch(switchcom_400){
			case 0:
				uccommand_pccode=0; // code initial
				if(get_char0 == PC_HEAD1) { // 송신 해드1	FB
					switchcom_400=3;// next
				}
				else switchcom_400=0;// Home
				break;	
			case 3:						
				uccommand_pccode = get_char0;// 조이스틱 정보 읽음 Code 
				switchcom_400=0;// Home
				GPIO_TOGGLE(LEDGP);// toggled LED
				setflag(F_ACT_400);  // 
				clrflag(F_COMTIME_400);
			break;			
		}				
	}	
}
...
unsigned char check_pop0(void){
	if(q0.wp != q0.rp){
		get_char0=(q0.buffer[q0.rp]);	// pop get char by check flag
		q0.rp++;						// Increment Read Point
		if(q0.rp> QSIZE)	q0.rp=0;	// Read 번지증가
		return 1;		
	}  //end if                      
	return 0;
} //end check_pop
...
unsigned char check_pop1(void){
	if(q1.wp != q1.rp){
		get_char1=(q1.buffer[q1.rp]);	// pop get char by check flag
		q1.rp++;						// Increment Read Point
		if(q1.rp> QSIZE)	q1.rp=0;	// Read 번지증가
		return 1;		
	}  //end if                      
	return 0;
} //end check_pop
...
unsigned short rcountx;
	...
	for(rcountx=0; pui8Buffer[rcountx]!='\0'; rcountx++){
		if(pui8Buffer[rcountx]=='\n'){
			...
			UART_WRITE(UART0, pui8Buffer[rcountx]);			
		}
	}	
}	
...
unsigned short rcountx;
	...
	for(rcountx=0; pui8Buffer[rcountx]!='\0'; rcountx++){
		if(pui8Buffer[rcountx]=='\n'){
			//UART_WRITE(UART1, 0x0a);	// 
			//UART_WRITE(UART1, 0x0d);			
		}
		else {
